...
document.querySelector("#joinButton").addEventListener("click", async (event) => {
    if(!("geolocation" in navigator)){
        Alert.alert("Kan geen locatie ophalen. Herlaad de pagina en probeer het opnieuw.");
    }
    Alert.alert("Locatie controleren...");
    navigator.geolocation.getCurrentPosition(attemptConnection, locationError, {enableHighAccuracy:true});

    async function locationError(error){
        Alert.alert(`Kan geen locatie ophalen: ${error.message}`);
    }
